...
def parse_parameters_block(block: str) -> dict:
    ...
    while i < len(lines):
        ...
        if m:
            ...
            nullable = ("None" in typ) or (
                default is None
                and (desc or "").lower().find("default none") >= 0
            )
            res[name] = {
...
def summarize(symbol: str) -> dict:
    ...
    try:
        ...
        # --- NEW: extract module-level imports for later reconstruction ---
        ...
        raises_list = [
            ln.strip("-• \t")
            for ln in sections.get("Raises", "").splitlines()
            if ln.strip()
        ] or None
        if not raises_list:
            found = sorted(
                set(
                    ...
                    for m in re.finditer(
                        r"\b([A-Z][A-Za-z]+Error)\b", docstring
                    )
                )
            )
        ...
        notes_list = [
            ln.strip()
            for ln in sections.get("Notes", "").splitlines()
            if ln.strip()
        ] or None
        examples_code = extract_examples_code(
        ...
        return {
            ...
            "module_imports": module_imports,  # NEW: for rebuilding the module
