...
This module performs mutation analysis on a target Python function
(e.g., pandas.concat) using test suites generated by multiple LLMs.

Pipeline summary:
    1. Load the extracted function body from a JSON spec (produced by fetch.py).
    2. Generate mutants by applying simple pattern-based mutations.
    3. For each LLM-generated test suite:
         - Run baseline tests (no mutation) to detect pre-existing failures.
         - Apply each mutant and rerun tests.
         - Track whether the test suite detects (kills) each mutant.
    4. Compute mutation score:
         mutation_score = killed / total_mutants
    5. Save a JSON report per model inside mutation_results/.

This script does NOT modify logic—only analyzes test effectiveness.
"""
...
# Loading and Mutant Generation

def load_body_from_json(spec_path: str) -> str:
    """
    Load the stripped function body from a JSON specification generated by fetch.py.
    """
...
def generate_mutants_from_body(body: str) -> List[Mutant]:
    """
    Generate simple text-based mutants using pattern-replacement rules.

    This is NOT a full mutation engine—it intentionally uses lightweight,
    operator-level mutations to evaluate the relative strength of generated tests.
    """
    mutants: List[Mutant] = []
...
# Mutation Module Builder

def build_module_source_from_mutated_body(mutated_body: str) -> str:
    """
    Wrap the mutated function body into a standalone Python module
    that overrides pandas.concat during test execution.
    """
...
def using_copy_on_write():
    \"\"\"Compatibility helper for older pandas versions.\"\"\"
    try:
...
def write_conftest_for_concat(project_root: str, module_name: str, function_name: str):
    """
    Create a temporary conftest.py that monkey-patches pandas.concat
    to point to the mutated version under test.
    """
...
def evaluate_one_llm_from_json(
    ...
    """
    Evaluate a single LLM-generated test suite by:
        1. running a baseline test
        2. running all mutants
        3. counting killed vs survived mutants
    """
...
# High-Level Orchestrator Across All Models
...
def evaluate_generated_tests_from_json_baseline(
    ...
    """
    Evaluate ALL LLM-generated test suites located in generated_dir.

    For each test file:
        - perform mutation analysis
        - print results
        - save JSON output inside results_dir/
    """
